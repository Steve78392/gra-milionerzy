...
try:
 ...
 try:
  import g4f
     ...
     print('Nie można zainstalować wymaganej do działania gry biblioteki - gpt4free. Zamykanie gry.')
     sys.exit()
...
def generuj_pytanie(stawka):
    try:
        response = g4f.ChatCompletion.create(
            model='gpt-4',
            messages=[{"role": "user", "content": '''Generuj pytanie do gry "Milionerzy" z podanymi odpowiedziami A, B, C, D oraz poprawną odpowiedzią z zachowaniem formatu: ```
...
def main():
    ...
    while True:
        ...
        if isinstance(pytanie, dict) and isinstance(old_pytanie, dict) and 'question' in pytanie and 'question' in old_pytanie:
        ...
        if pytanie:
            ...
            if isinstance(pytanie, dict) and 'correct' in pytanie:
                ...
                if odpowiedz == poprawna_odpowiedz:
                    ...
                    print(f"Niestety, zła odpowiedź, prawidłowa odpowiedź to odpowiedź {pytanie['correct']}. Koniec gry.")
                    break
            else:
                print("Nieprawidłowy format pytania. Spróbuj ponownie.")
        else:
            print('Nie wygenerowano pytania.')
